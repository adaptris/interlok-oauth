 ...
 * and {@link ApacheHttpProducer}.
 * <p>
 * This has been verified to work with NetSuite OAUTH 1.0 (using HMAC-SHA1), but not any other OAUTH
 * 1.0 providers.
 * </p>
 * <p>
...
public class ApacheRfc5849Authenticator implements ApacheRequestAuthenticator {
  ...
  public ApacheRfc5849Authenticator() {
  ...
  public void configure(HttpRequestBase req) throws Exception {
    req.addHeader(HttpConstants.AUTHORIZATION,
        builder.withMethod(req.getMethod()).withUrl(req.getURI().toURL()).build());
  }
